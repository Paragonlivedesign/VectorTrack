...
Creates invoice text in the drawing
"""
...
def TimeTrackerSimple():
    ...
    # Ask if they want to create invoice in drawing
    if vs.YNDialog(f"Create invoice text for {project_name}?") == 1:
        # Create invoice content
        invoice_lines = []
        invoice_lines.append(f"INVOICE - {project_name}")
        invoice_lines.append(f"Generated: {datetime.now().strftime('%m/%d/%Y %I:%M %p')}")
        invoice_lines.append("=" * 60)
        invoice_lines.append("")
        invoice_lines.append(f"Project: {project_name}")
        invoice_lines.append(f"Total Hours: {total_hours:.2f}")
        invoice_lines.append(f"Hourly Rate: ${rate:.2f}")
        invoice_lines.append(f"TOTAL DUE: ${total_amount:.2f}")
        invoice_lines.append("")
        invoice_lines.append("=" * 60)
        invoice_lines.append("")
        invoice_lines.append("Session Details:")
        for start, end, hours in sessions:
            invoice_lines.append(f"{start.strftime('%m/%d/%Y %I:%M %p')} - {end.strftime('%I:%M %p')} ({hours:.2f} hrs)")
        
        invoice_text = '\n'.join(invoice_lines)
        
        # Create text at 0,0 on current layer
        vs.MoveTo(0, 0)
        text_handle = vs.CreateText(invoice_text)
        
        # Select the text
        vs.DSelectAll()
        vs.SetSelect(text_handle)
        
        # Get current layer name
        current_layer_handle = vs.ActLayer()
        current_layer_name = vs.GetLName(current_layer_handle)
        
        vs.AlrtDialog(f"Invoice created at 0,0 on layer '{current_layer_name}'!\n\n" + 
                     "The text is selected - you can:\n" +
                     "• Move it to another location\n" +
                     "• Cut and paste to a different layer\n" +
                     "• Create an 'Invoice' sheet layer and move it there")
